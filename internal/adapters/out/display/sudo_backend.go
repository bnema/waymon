...
func newSudoBackend(ctx context.Context) (Backend, error) {
	log := zerolog.Ctx(ctx)
	...
	log.Debug().Msg("sudo backend available")
	return &sudoBackend{}, nil
}

func (s *sudoBackend) GetMonitors() ([]*domain.Monitor, error) {
...
`
	...
	if err := json.Unmarshal(output, &monitors); err != nil {
		...
		if outputStr != "" {
			return nil, fmt.Errorf("failed to parse monitor data: %w (output: %s)", err, outputStr)
		}
	...
	validMonitors := make([]*domain.Monitor, 0, len(monitors))
	...
	determinePrimaryMonitor(validMonitors)
